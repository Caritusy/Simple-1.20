...
public:
	...
	static __forceinline unsigned __int32 rotBy(int in, unsigned int by) {
		auto mut = static_cast<unsigned __int32>(in);
	...
	static void DownloadFile(const std::string& url, const std::string& local_path);
	...
	static float currentTimeMS() {
		auto t = std::chrono::high_resolution_clock::now().time_since_epoch();
		return std::chrono::duration<float>(t).count();
	}

	static float getTimeHue(float moveSpeed, float speed, float offset) {
		return fmodf(currentTimeMS() * speed - moveSpeed - offset, 1);
	}
};
