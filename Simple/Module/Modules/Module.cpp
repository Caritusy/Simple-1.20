...
void IModule::registerStringSetting(std::string name, std::string* stringPtr, std::string defaultValue){
	SettingEntry* setting = new SettingEntry();
	setting->valueType = ValueType::TEXT_T;

	setting->value = reinterpret_cast<SettingValue*>(stringPtr);  // Actual value

	SettingValue* defaultVal = new SettingValue();  // Default Value
	defaultVal->text = new std::string(defaultValue);
	setting->defaultValue = defaultVal;
...
//void IModule::registerStringSetting(std::string name, std::string* strptr)
//{
//	SettingEntry* setting = new SettingEntry();
//	setting->valueType = ValueType::TEXT_T;
//
//	setting->value = reinterpret_cast<SettingValue*>(strptr);  // Actual value
//
//	SettingValue* defaultVal = new SettingValue();  // Default Value
//	defaultVal->text = strptr;
//	setting->defaultValue = defaultVal;
//
//	strcpy_s(setting->name, 19, name.c_str());  // Name
//
				...
				try {
					...
					case ValueType::ENUM_T:
						try {
							...
							LOG(INFO) << "Config Load Error(Enum)" << this->getRawModuleName() << e.what();
						}
						break;
					}
...
#endif
			Game.getClientInstance()->getGuiData()->displayClientMessageF("%s%s >> %s", enabled ? GREEN: RED, this->getModuleName(), enabled ? (const char*)u8"Enabled ✓" : (const char*)u8"Disabled ✕");
		//LOG(INFO) << (enabled ? "Enabled " : "Disabled ") << this->getModuleName();
		if (enabled)
			this->onEnable();
		else
			this->onDisable();
...
void IModule::clientMessageF(const char* fmt, ...) {
	...
	GameData::log("[%s]: %s", this->getRawModuleName(), message);
...
void SettingEntry::makeSureTheValueIsAGoodBoiAndTheUserHasntScrewedWithIt() {
	...
	case ValueType::TEXT_T:
		//break;
	default:
		LOG(INFO)<<"unrecognized value "<< (int)valueType;
		break;
	}
}
