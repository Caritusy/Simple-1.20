...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    draw->AddText(ImVec2(total_bb.Min.x, frame_bb.GetCenter().y - label_size.y / 2), GetColorU32(ImGuiCol_Border), label);

    draw->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(ImGuiCol_Border), 5);
    draw->AddRectFilled(frame_bb.Min, ImVec2(frame_bb.Min.x + value->second, frame_bb.Max.y), GetColorU32(ImGuiCol_Border), 5);
    draw->AddCircleFilled(ImVec2(frame_bb.Min.x + value->second - 2, frame_bb.GetCenter().y), 6, GetColorU32(ImGuiCol_Border), 30);

    draw->AddRectFilled(number_bb.Min, number_bb.Max, GetColorU32(ImGuiCol_Border), 4);
    draw->AddRect(number_bb.Min, number_bb.Max, GetColorU32(ImGuiCol_ChildBg), 4);

    if (IsMouseHoveringRect(number_bb.Min, number_bb.Max) && CalcTextSize(value_buf, value_buf_end).x > number_bb.GetWidth())
        GetForegroundDrawList()->AddText(ImVec2(number_bb.GetCenter().x - CalcTextSize(value_buf, value_buf_end).x / 2, number_bb.Min.y + 1), GetColorU32(ImGuiCol_Text), value_buf, value_buf_end);
    else
        draw->AddText(number_bb.Min + ImVec2(2, 1),GetColorU32(ImGuiCol_Text), value_buf);
